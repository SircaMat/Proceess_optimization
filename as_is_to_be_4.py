...
<div class=\"section\"><p>{{left_people}}</p></div>
<div class=\"section\"><p>{{left_process}}</p></div>
<div class=\"section\"><p>{{left_technology}}</p></div>
<div class=\"section\"><p>{{left_data}}</p></div>
<div class=\"section\"><p>{{left_output}}</p></div></div>
<div class=\"panel right\"><div class=\"side-label\"><span class=\"tag\">{{right_label}}</span></div>
<div class=\"section\"><p>{{right_people}}</p></div>
<div class=\"section\"><p>{{right_process}}</p></div>
<div class=\"section\"><p>{{right_technology}}</p></div>
<div class=\"section\"><p>{{right_data}}</p></div>
<div class=\"section\"><p>{{right_output}}</p></div></div>
...
def _get_translator_or_none():
    try:
        from deep_translator import GoogleTranslator  # optional
...
def apply_fields_to_html(original_html: str, fields: Dict[str, str]) -> str:
    ...
    # Sections
    def set_sec(panel_sel, idx, value):
    ...
    # Footer
    set_text("footer .pagenum", fields["page_number"]) 
    set_text("footer strong", fields["footer_summary"]) 
...
def persist(fields: Dict[str, str]):
    try:
        STATE_FILE.write_text(json.dumps(fields, ensure_ascii=False, indent=2), encoding="utf-8")
    except Exception:
        pass
...
def init_fields(fields: Dict[str, str]):
...
st.title("HTML Slide Editor – IRRBB / ALM")
st.caption("Upload an HTML slide (template-based), edit fields, translate, and export. Robust on re-uploads and clears.")
...
with st.sidebar:
    ...
    if st.button("🌐 Translate current fields", use_container_width=True):
        ...
        st.toast(f"Translated to {lang.upper()} (best-effort)")
...
# Ensure base_html exists for preview/export
base_html = st.session_state.get("base_html", FALLBACK_HTML)

# If first load with no widgets yet, initialize defaults to ensure deterministic values
if not any(k in st.session_state for k in FIELD_KEYS):
    init_fields(DEFAULTS.copy())

# -----------------------------
# Editor Form (values are driven from session_state)
# -----------------------------
...
preview_html = apply_fields_to_html(base_html, current)
