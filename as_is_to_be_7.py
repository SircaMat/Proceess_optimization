...
<div class="panel user" style="background:#dcdcdc; margin-top:16px">
  <div class="section"><p>{{user_notes}}</p></div>
</div>
...
def parse_html_fields(html: str) -> Dict[str, str]:
    ...
    fields: Dict[str, str] = {
        ...
        "user_notes": user_section.get_text(strip=True) if user_section else "",
...
with st.form("editor_form", clear_on_submit=False):
    ...
    # User notes editor (grey panel content)
    st.text_area("User notes (grey panel)", key="user_notes")
